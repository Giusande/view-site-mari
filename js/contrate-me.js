...
const captacaoCampos = document.getElementById("captacaoCampos");
...
const quantidadeVideos = document.getElementById("quantidadeVideos");
const descricaoCaptacao = document.getElementById("descricaoCaptacao");
...
tipoServico.addEventListener("change", () => {
  ...
  captacaoCampos.classList.add("hidden");

  if (tipoServico.value === "Storymaker") {
    storymakerCampos.classList.remove("hidden");
  }

  if (tipoServico.value === "Captacao") {
    captacaoCampos.classList.remove("hidden");
  }
});
...
continuarBtn.addEventListener("click", () => {
  if (!tipoServico.value) {
    alert("Selecione o tipo de serviço antes de continuar.");
    return;
  }
...
formPessoal.addEventListener("submit", (e) => {
  ...
  if (tipoServico.value === "Captacao") {
    mensagem += `🎬 Quantidade de vídeos: ${quantidadeVideos.value}%0A`;
    mensagem += `📝 O que você precisa: ${descricaoCaptacao.value}%0A`;
  }
